...
<div class="">
			...
			<div class="x_panel">
				<div class="x_title">
					...
					<h2><?php if(isset($_GET['viewhistory'])) { 
					  ...
					  echo "User History(Flat)";}else {echo "User's Pages"; }?></h2>
	...
	//echo $rowe['0']['amt_financed'];
		...
		// echo $emim;
		  ...
		  $rprincipalf = $rowe['0']['amt_financed'];
		  $penalty = $rowe['0']['penalty'];
		  $overduecharges = $rowe['0']['overdue_charges'];
		  $interestf = $rprincipalf*$mper;
		  //echo $interestf;
		  //echo  $period ;
		 // die();
		  $totalamount = round($interestf)*$period+$rowe['0']['amt_financed'];
		  //echo $totalamount;
		 // die();
				  
		  $emic =  round($totalamount/$period);
		  $fprincipal =  $emic-$interestf;
		  //die();
		  ...
		  for($i = 1;$i<=$period;$i++)
		  {
			...
			else
			{
					...
					foreach($rowf as $rowam)
					{
					 ...
					 // $fin_date_c = date('d',strtotime($rowam['finance_date']));
					 ...
					 $calculatep["20".$year][$months]['rpr'] =  $emic-$calculatep["20".$year][$months]['interest'];
					 	if(isset($calculatep["20".$year][$months]['amountr'])  && $calculatep["20".$year][$months]['amountr']>=$emic)
							{
	...
	foreach($calculatep["20".$year] as $mont) 
	{
	 $installt++;
	   ...
	   {		  
		...
		if($emic-$mont['amountr']>2 &&  $mont['day']<date('d') && $year<=date('y')&& date('m')>=date('m',strtotime($mont['mone']))) { //$penaltyt["20".$year]+=$penalty;
					 $install++;
		}
		if($emic-$mont['amountr']<2 &&  $mont['day']<date('d') && $year<=date('y')&& date('m')>=date('m',strtotime($mont['mone']))) { //$penaltyt["20".$year]+=$penalty;
					// $installp++;
		}
		if($emic-$mont['amountr']<2) {
					 $installp++;
		}
	}
		 ...
		 //echo $pen_fin;
		 //die();
		 $installmentanet =   $tpenalty+$tinstallmentdue; 
		 $calculatepytps = ceil($interestf*$installp);	
		 $calculatepyts = floor($fprincipal*$installp);
		 if($finstallmentpaid == 0 && $tinstallmentdue == 0 &&  $tpenalty == 0){ $status = "Close";}else{$status = "Active";}
	...
	<tr>
	    ...
	    <td  id = "ptad"><b>Instalment Overdue (Rs)</b> </td>
	...
	<div>
		<table>
		<tr>
			<th id = "ptad">Year</th><th id = "ptad">Principal</th><th id = "ptad">Interest</th><th id = "ptad">EMI</th><th id = "ptad">Received Amount</th><th id = "ptad">Penalty</th><th id = "ptad">Overdue Charges</th><th id = "ptad">Balance</th>
			...
			?>
                                         ...
                                         <div class="panel">
                                            ...
                                            <div id="heading<?php  echo ""."20".$year; ?>" class="panel-collapse collapse" role="tabpanel" aria-labelledby="headingOne">
                                                <div class="panel-body">
                                                    ...
                                                    <table class="table table-bordered">
                                                       <thead>
                                                            <tr>
                                                              <th id = "ptad">Month</th><th id = "ptad">Principal</th><th id = "ptad">Interest</th><th id = "ptad">EMI</th><th id = "ptad">Recieved Amount</th><th>Penalty</th><th id = "ptad">Overdue Charges</th><th id = "ptad">Balance</th>
                                                            </tr>
                                                        </thead> 
                                                        <tbody>
														<?php 
											...
											<tr>
                                                               ...
                                                               <td id = "ptad"> <?php echo $mont['mone'];?> </td>
                                                               <td id = "ptad"> <?php echo floor($mont['rpr']);?> </td>
                                                               <td id = "ptad"> <?php echo floor($mont['interest']);?> </td>
															   <td id = "ptad"> <?php echo floor($mont['interest']+$mont['rpr']);?>     </td>
															   <td id = "ptad"><?php echo $mont['amountr'];?></td>
                                                               ...
                                                               <td id = "ptad"> <?php echo floor($mont['balance']);?> </td>
                                                            </tr>
														<?php	  		
														      } 
														?>
