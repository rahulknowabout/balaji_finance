...
$query = "select * from financeuser INNER JOIN financestatement ON financeuser.agreement_no = financestatement.agreement_no  where financeuser.agreement_no = '".$_GET['agreement_no']."'";
	...
	foreach($calculatep["20".$year] as $mont) 
	{
	   ...
	   {		  
		...
		if(floor($mont['interest']+$mont['rpr'])-$mont['amountr']<2) {
		 ...
		 //echo $pen_fin;
		 //die();
		 $installmentanet =  $tpenalty+$tinstallmentdue+$toverduecharges; 
		...
		if($tomonth<10){ $toinstallp = "".$install_tod."/0".$tomonth."/20".$toyear."";  } else { $toinstallp = "".$install_tod."/".$tomonth."/20".$toyear."";}
		 if($tomonth<10){ $toinstallps = "".$install_tod."/0".$tomonth."/20".$toyear."";  } else { $toinstallps = "".$install_tod."/".$tomonth."/20".$toyear."";}
		 $instaldate=date('d/m/Y',strtotime($rowe['0']['install_from']));
	...
	$pdf1 = "<u>Flat PDF</u><table><tr><h1>Balaji Finance</h1></tr></table><table>
 	<tr>
		<td>
 	<tr>
	...
	<tr>
		...
		<td  id = \"ptad\">Instalment Overdue (Rs) </td>
	...
	<tr>
		...
		<td id = \"ptad\"><b>".date('d/m/Y',strtotime($rowe['0']['disbursal_date']))."<b/></td>
		<td  id = \"ptad\">OD Charges Due(Rs) </td>
	...
	<tr>
		...
		<td  id = \"ptad\">Instalment Paid (Rs) </td>
	...
	<tr>
	<td  id = \"ptad\">Instl Period</td>
		<td  id = \"ptad\"><b>".$instaldate." to ".$toinstallp."</b></td>
	...
	<tr><td  id = \"ptad\">Instalment Plan</td>
	...
	<table  border=\"1\" style = \"border-collapse: collapse;border-top-style:none; width:750px;border-top-width:0;border-style:none;table-layout:fixed;font-size:11px\">
		...
		//die();
			...
			if($totalmonth<12)
			{
				...
				$counter++;
			$installment_date = "".$installment_day."/".$installment_month."/20".$installment_year."";
				...
				$pdf4 = $pdf4."<table  border=\"1\" style = \"border-collapse: collapse;border-top-style:none; width:750px;border-top-width:0;border-style:none;table-layout:fixed;font-size:11px\"><tr>
					...
					0
						...
						foreach($rowf as $rowd)
						{
							...
							if(date('y',strtotime($rowd['finance_date_f'])) == $installment_year)
							{
								if(date('m',strtotime($rowd['finance_date_f'])) == $installment_month)
								{
								 $ddss = date('d/m/Y',strtotime($rowd['finance_date_f']));
								 $financeday = date('d',strtotime($rowd['finance_date_f']));
								 $duepenalty=0;
					...
					0
						...
						$a++;
						  ...
						  if($installment_month==12)
								   ...
								   $installment_month++;
								   $installment_month="0".$installment_month;
				    }	
			...
			for($i = $installment_month;$i<=$totalmonth;$i++)
				...
				$counter++;
					...
					else
					{
						if($i<10)
						{
						$installment_date = "".$installment_day."/0".$i."/20".$installment_year."";
						}
						else
						{
							$installment_date = "".$installment_day."/".$i."/20".$installment_year."";
						}
					}
					$pdf4 = $pdf4."<table  border=\"1\" style = \"border-collapse: collapse;border-top-style:none; width:750px;border-top-width:0;border-style:none;table-layout:fixed;font-size:11px\"><tr>
					<td id = \"ptad\" width = \"75px\" >
					...
					".$rowd['od_recieve']."	
						...
						foreach($rowf as $rowd)
						{
							...
							if(date('y',strtotime($rowd['finance_date_f'])) == $installment_year)
							{
								if(date('m',strtotime($rowd['finance_date_f'])) == $i)
								{
								 //echo "assddff";
								 //die();
